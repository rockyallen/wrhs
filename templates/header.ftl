...
<html lang="en">
  <head>
    <meta charset="utf-8"/>
    <title><#if (content.title)??><#escape x as x?xml>${content.title}</#escape><#else>WRHS</#if></title>
    ...
    <!-- Le styles -->
    <!--link href="<#if (content.rootpath)??>${content.rootpath}<#else></#if>wrhs2/css/dark.css" rel="stylesheet"-->
    <link href="<#if (content.rootpath)??>${content.rootpath}<#else></#if>wrhs2/css/bootstrap.min.css" rel="stylesheet">   
